...
RSpec.describe ObsBridge::BridgeState do
  ...
  let(:status_writer) do
    instance_double(ObsBridge::StatusWriter).tap do |writer|
      allow(writer).to receive(:write_snapshot) do |payload|
        snapshots << payload
      end
    end
  end
  ...
  it "boots disabled and down, and projects that into the status writer" do
    expect(state.desired_enabled?).to be(false)
    expect(state.runtime_connected?).to be(false)
    expect(state.runtime_state).to eq("down")

    expect(status_writer).to have_received(:write_snapshot).with(
      hash_including(
        "bridge_id" => "main",
  ...
  it "can be enabled and disabled" do
    ...
    expect(latest_snapshot["desired_state"]).to eq("enabled")

    state.disable!
    expect(state.desired_enabled?).to be(false)
    expect(latest_snapshot["desired_state"]).to eq("disabled")
  end
  ...
  it "tracks connection transitions" do
    ...
    expect(latest_snapshot["connected"]).to eq("true")
    expect(latest_snapshot["runtime_state"]).to eq("up")
  ...
  it "records a capture window and reports activity while it is live" do
    ...
    expect(latest_snapshot["capture_all_active"]).to eq("true")
    expect(Time.iso8601(latest_snapshot["capture_all_until"])).to eq(Time.utc(2026, 3, 23, 18, 15, 0))
  end

  it "extends a capture window rather than shrinking it" do
    state.capture_all_for(900)
    first_deadline = Time.iso8601(latest_snapshot["capture_all_until"])

    clock_state.now += 60
    state.capture_all_for(30)

    expect(Time.iso8601(latest_snapshot["capture_all_until"])).to eq(first_deadline)
  end
  ...
  it "shows capture-all as inactive after expiry once status is refreshed" do
    ...
    state.heartbeat!
    expect(latest_snapshot["capture_all_active"]).to eq("false")
  end
  ...
  it "can clear the capture window" do
    ...
    expect(state.capture_all_active?).to be(false)
    expect(latest_snapshot["capture_all_until"]).to eq("")
    expect(latest_snapshot["capture_all_active"]).to eq("false")
  end

  it "records and clears errors" do
    state.set_last_error!("kaboom")
    expect(latest_snapshot["last_error"]).to eq("kaboom")

    state.clear_last_error!
    expect(latest_snapshot["last_error"]).to eq("")
  end

  it "records heartbeats" do
    state.heartbeat!
    expect(latest_snapshot["last_heartbeat_at"]).to eq("2026-03-23T18:00:00.000000Z")

    clock_state.now += 5
    state.heartbeat!
    expect(latest_snapshot["last_heartbeat_at"]).to eq("2026-03-23T18:00:05.000000Z")
  end
